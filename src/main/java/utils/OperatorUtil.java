    ...
    public static ScriptEngine getEngine() {
        if (OperatorUtil.engine == null) {
            try {
                ...
                (OperatorUtil.engine = manager.getEngineByName("javascript")).getBindings(100);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        } else {
            OperatorUtil.engine.createBindings();
        }
        return OperatorUtil.engine;
    }
